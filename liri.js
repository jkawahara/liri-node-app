...
function requestEvents(artistName) {
  ...
  .then(function (response) {
    ...
    // Store event information in array for logging
    var eventLog = [];
    eventLog.push('Upcoming events:');
  ...
  .then(function(response) {
    ...
    for (var i = 0; i < dataJSON.length; i++) {
      ...
      songLog.push(
        ...
        'Song: ' + song.name + '\n' +
        'Preview Link: ' + song.album.external_urls.spotify + '\n' +
...
function readRandomFile() {
  fs.readFile('random.txt', 'utf8', function(error, data) {
    ...
    if (error) {
      ...
      console.log('Random.txt says: ' + queryArray[0] + ' "' + queryArray[1] + '"');
...
// Run app by calling readCL() and passing in command line arguments (process.argv[2], [3]...)
